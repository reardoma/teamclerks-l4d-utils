...
public _GT_TankSpawn_Event()
{
	...
	if (IsFakeClient(client)) // if AI tank
	{
		...
		CreateTimer(fSelectionTime, _GT_ResumeTank_Timer);	// Create timer for restoring ai tank
		fFireImmunityTime += fSelectionTime;				// Add some more time to fire immunity
	}

	CreateTimer(0.1, _GT_TankOnFire_Timer, _, TIMER_REPEAT);
...
public _GT_OnPlayerRunCmd(client, &buttons, &impulse, Float:vel[3], Float:angles[3], &weapon)
{
	...
	if (g_iBlockClientThrow == client && buttons & IN_ATTACK2)
	{
 ...
 * @noreturn
 */
public Action:_GT_ResumeTank_Timer(Handle:timer)
{
	g_iBlockClientThrow = 0; // Reset throw block

	new client = GetTankClient(); // Get tank client
	if (!client) return; // If tank client isn't valid, return
 ...
 * @noreturn
 */
public Action:_GT_TankOnFire_Timer(Handle:timer)
{
	if (!IsTankInPlay() || !g_bIsTankFireImmune) return Plugin_Stop;

	new client = GetTankClient();
	if (!client) return Plugin_Stop;

	if (!IsTankOnFire()) return Plugin_Continue;

	ExtinguishEntity(client);
	AddTankClientToInfHUD();

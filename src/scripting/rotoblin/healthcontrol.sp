...
enum HEALTH_STYLE
{
	...
	SAFEROOM_KITS_ONLY = 4 // Replace the finale kit with pills, remove all other pills/kits, give players a kit upon leaving the saferoom.
}
...
public _HealthControl_OnPluginStart()
{
	...
	g_hHealthStyle_Cvar = CreateConVarEx("health_style", 
		buffer, 
		"How medkits and pills will be configured. 0 - Don't replace any medkits, 1 - Replace all medkits with pills, 2 - Replace all but finale medkits with pills, 3 - Replace safe room and finale kits with pills; remove all other health sources, 4 - Replace finale kits with pills, remove all other health source, give players kits upon leaving the safe room.", 
